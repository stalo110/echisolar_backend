export function calculateInstallments(total: number, months: number) {
  ...
  if (diff !== 0) schedule[schedule.length - 1].amount = Number((schedule[schedule.length - 1].amount + diff).toFixed(2));
